<div ng-controller="SidebarCtrl" ng-init="postId = <?= $post->ID;?>" >
    ...
    <ul ng-show="dictConfig.enabled && glossary" class="tat-sidebar-glossary"  >
        <li ng-repeat="(name, term) in glossary" ng-class = "{ 'tat-sidebar-term-dirty': !term.id }">
            ...
            <a href="" ng-click="openDictionary($event, name)" title="{{term.description}}">{{ name }} <span ng-if="!term.parent_name && term.id > 2000000" >( ? )</span></a>
            <a href="" ng-if="term.parent_name" ng-click="openDictionary($event,term.parent_name)" title="{{term.parent_description}}">({{ term.parent_name }})</a>
        </li>
    </ul>

    <div ng-show="dictConfig.enabled" class="tat-popup-button-block" ng-click="openDictionary($event)" ng-swipe-left="openDictionary($event)">
